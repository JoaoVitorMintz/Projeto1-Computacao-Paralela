...
### Análise dos sensores de forma paralela otimizada (Etapa 4):

O problema central da versão com mutex é a **contenção**: toda vez que uma thread quer atualizar os dados globais (vetor de sensores, contador de alertas, consumo total), ela precisa travar o mutex e esperar todas as outras liberarem. Com muitas threads lendo linhas ao mesmo tempo e todas brigando pelo mesmo lock, o paralelismo praticamente some e o programa pode até ficar mais lento do que a versão com menos threads.

A solução implementada no sensor_analyzer_par_otimizado.c foi usar **acumuladores locais por thread**: em vez de todas as threads compartilharem e disputarem as mesmas estruturas globais, cada thread recebe sua própria cópia local do array de sensores, do consumo total e do contador de alertas (alocados via calloc dentro da struct ThreadArgs). Durante todo o processamento, nenhuma thread precisa de mutex, pois nenhuma delas toca nos dados das outras. Quando todas terminam (após o pthread_join), o programa principal percorre os resultados locais de cada thread e faz a fusão (**merge**) no array global, como nesse ponto só existe uma "thread" rodando (a main), não há risco de race condition e nenhum lock é necessário.

O trade-off é de **memória vs. velocidade**: cada thread precisa de um array próprio de `MAX_SENSORES` structs, então o consumo de memória cresce com o número de threads. Mas o ganho de desempenho compensa: sem contenção de mutex, as threads trabalham em paralelo de verdade.
